...
try:
    ...
    raise ImportError("Please install DALI to run this example.")
import os
os.environ["NCCL_DEBUG"] = "INFO"
#V ERSION - Prints the NCCL version at the start of the program.
...
parser.add_argument('--arch', '-a', metavar='ARCH', default='zerosnet18_in',
                    choices=model_names,
                    help='model architecture: ' +
                    ' | '.join(model_names) +
                    ' (default: zerosnet18_in)')
...
parser.add_argument('--opt_level', default='O2', type=str,
                    help='O2 is fast mixed FP16/32 training, O0 (FP32 training) and O3 (FP16 training), O1 ("conservative mixed precision"), O2 ("fast mixed precision").--opt_level O1 and O2 both use dynamic loss scaling by default unless manually overridden. --opt-level O0 and O3 (the "pure" training modes) do not use loss scaling by default. ')
...
parser.add_argument("--notes", default='_', type=str)
...
parser.add_argument('--k_ini', type=float, default=-1.8)
parser.add_argument('--share_k', type=bool, default=False)
...
if args.given_coe is not None:
    ...
    given_coe_text = ''
if args.share_k:
    share_k_text = 'share_k_True'
else:
    share_k_text = 'share_k_False'

args.save_path = 'checkpoints/imagenet/zerosnet/'+args.arch+'_opt_level_'+mixed_type+\
                                    'PL' + str(args.PL) + 'k_ini_' + str(
    args.k_ini)  + given_coe_text + share_k_text+ '_sche_' + args.sche +'_opt'+str(args.opt)+'BS'+str(args.batch_size)+'LR'+\
                                   str(args.lr)+'n_gpu_'+str(args.n_gpu)+'epochs'+\
                                   str(args.epochs)+'warmup'+str(args.warmup)+\
...
if args.distributed:
    ...
    try:
        ...
        raise ImportError("Please install apex to run this example.")
...
def main():
    ...
    if args.pretrained:
        ...
        if 'zerosnet' in args.arch:
            print('is zerosnet')
            model = models.__dict__[args.arch](PL=args.PL, k_ini=args.k_ini, given_coe=args.given_coe, share_k = args.share_k)
        else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.local_rank == 0:
            ...
            Start_time = "{0:%Y-%m-%dT%H-%M-%S/}".format(datetime.now())
        ...
        if args.local_rank == 0:
            End_time = "{0:%Y-%m-%dT%H-%M-%S/}".format(datetime.now())
...
def train(train_loader, model, criterion, optimizer, epoch, scheduler):
    ...
    top5 = AverageMeter()
    if 'zerosnet' in args.arch:
        stepsize = AverageMeter()
        if args.given_coe is None:
            ks = []
            if not args.share_k:
                for j in range(4):
                    ks = ks+[AverageMeter()]
    ...
    for i, data in enumerate(train_loader):
        ...
        # compute output
        if 'zerosnet' in args.arch:
            output, ks_step, stepsize_step = model(input_var)
        else:
        ...
        if args.distributed:
            ...
            if 'zerosnet' in args.arch:
                if not isinstance(stepsize_step, int):
                    stepsize_step = reduce_tensor(stepsize_step)
        else:
        ...
        # print('stepsize_step', type(stepsize_step))

        # if not isinstance(stepsize_step, int):
        #     stepsize_step = stepsize_step.data.cpu().numpy()
        if 'zerosnet' in args.arch:
            if not isinstance(stepsize_step, int):
                stepsize.update(to_python_float(stepsize_step), input.size(0))
            if args.given_coe is None:
                for j in range(len(ks)):
                    ks[j].update(to_python_float(ks_step), input.size(0))
        ...
        # print('args.local_rank, args.print_freq, i', args.local_rank, args.print_freq, i)
        ...
        # if args.local_rank == 0:
            ...
            # print('train_loss_step', to_python_float(reduced_loss), global_step)
        ...
        if 'zerosnet' in args.arch:
            if not isinstance(stepsize_step, int):
                stepsize_step = stepsize_step.data.cpu().numpy()
                writer.add_scalar('stepsize_step', float(stepsize_step), global_step)
            if args.given_coe is None:
                if ks_step != 9999:
                    # print('ks1', ks)
                    # ks = np.array(ks)
                    # print('ks2', ks)
                    # if len(ks) == 1:
                    if isinstance(ks_step, float):
                        # ks = ks.cpu().detach().numpy()
                        # print('ks_step', ks_step)
                        writer.add_scalar('ks_step', ks_step, global_step)
                    else:
                        for j in range(len(ks_step)):
                            # print(i)
                            # print('i, ks.shape[0]', i, ks.shape[0])
                            # print('ks[i][0]', ks[i][0].data)
                            # print('ks[i]', float(ks[i][0].data.cpu().numpy()) )
                            # ks = float(ks_step[i].data.cpu().numpy())
                            # print('ks_step, i: ', ks_step, i)
                            # writer.add_scalar('ks_step_' + str(i), ks_step, global_step)
                            # print('ks_step[i], i: ', ks_step[i], i)
                            if not isinstance(ks_step[j], float):
                                ks_step[j] = float(ks_step[j].data.cpu().numpy())
                            writer.add_scalar('ks_step_' + str(j), ks_step[j], global_step)
                        # if i % args.print_freq == 0:
                        #     print('ks_step_' + str(j), ks_step[j])
...
#        writer.add_histogram(name+'/grad', param.grad, epoch)
    ...
    if 'zerosnet' in args.arch:
        if not isinstance(stepsize_step, int):
            writer.add_scalar('stepsize', stepsize.avg, epoch)
        # print('\nstepsize:', stepsize.avg)
        if args.given_coe is None:
            for j in range(len(ks)):
                # if not isinstance(ks[j], float):
                #     ks[j] = float(ks[j].data.cpu().numpy())
                writer.add_scalar('ks_' + str(j), ks[j].avg, epoch)
        else:
            writer.add_scalar('a_0', args.given_coe[0], epoch)
            writer.add_scalar('a_1', args.given_coe[1], epoch)
            writer.add_scalar('a_2', args.given_coe[2], epoch)
            writer.add_scalar('b_0', args.given_coe[3], epoch)
